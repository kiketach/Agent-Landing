...
app = Flask(__name__)
CORS(app) # Habilita CORS para todas las rutas. Ajusta en producción.
...
if not all([PROJECT_ID, LOCATION, AGENT_ENGINE_RESOURCE_NAME]):
    logger.error("PROJECT_ID, LOCATION, and AGENT_ENGINE_RESOURCE_NAME environment variables are required.")
    # Podrías salir o manejar esto de otra forma si el engine no es crítico al inicio.
